...
async function getInfoData(paramQurey, userInfo) {
  ...
  for (const part of result) {
    ...
    if (userData) {
      ...
      userData.title = part.title;
      userData.descript = part.descript;
      userData.location = part.location;
...
async function updateUserStatus(userId, maritalSts, status) {
  const filter = { _id: new ObjectId(userId) };
  const option = { upsert: true };
  const setCls = {
    $set: {
      marital_status: maritalSts,
      status: status,
    },
  };
  ...
  try {
    ...
    if (result.modifiedCount > 0) {
      ...
      if (coupleResult.insertedId) {
        await updateUserStatus(metData.userId, "Married", "successful");
        await updateUserStatus(metData.partner, "Married", "successful");
      }
    }
    res.send(result);
  } catch (err) {
