...
const paymentHistoryCollection = mongoClient
  .db("SoulMate")
...
router.post("/order", async (req, res) => {
  ...
  const data = {
    ...
    currency: "BDT",
    tran_id: train_id,
    success_url: `https://soulmates-server.vercel.app/payment/success/${train_id}`, //server link
    fail_url: `https://soulmates-server.vercel.app/payment/fail/${train_id}`, //server link
  ...
  router.post("/payment/success/:tranId", async (req, res) => {
    ...
    if (result.modifiedCount > 0) {
      ...
      res.redirect(
        `https://soulmate-matrimony.netlify.app/payment/success/${req.params.tranId}` //client link
      );
    }
  });
